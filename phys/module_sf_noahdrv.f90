...
CONTAINS
   ...
   REAL, DIMENSION(its:ite,jts:jte) ::EMISS_URB,EMISS_URB0 
   REAL, DIMENSION(its:ite,jts:jte) :: RL_UP_URB,RL_UP_URB0 
   REAL, DIMENSION(its:ite,jts:jte) ::RS_ABS_URB
   REAL, DIMENSION(its:ite,jts:jte) ::GRDFLX_URB
   REAL :: SIGMA_SB,RL_UP_RURAL,RL_UP_TOT,RS_ABS_TOT,UMOM,VMOM,RL_UP_TOT0 
   REAL :: r1,r2,r3
  ...
  anth_diurnal(8)=0.80
  anth_diurnal(9)=0.87
  anth_diurnal(10)=0.81
  anth_diurnal(11)=0.82
  anth_diurnal(12)=0.85
  anth_diurnal(13)=0.90
  anth_diurnal(14)=0.90
  anth_diurnal(15)=0.94
  anth_diurnal(16)=1.00
  anth_diurnal(17)=0.89
  anth_diurnal(18)=0.78
  anth_diurnal(19)=0.56
  anth_diurnal(20)=0.49
  anth_diurnal(21)=0.45
  anth_diurnal(22)=0.42
  anth_diurnal(23)=0.37
  anth_diurnal(24)=0.32
   ...
   ENDDO JLOOP                                                          
      ...
      IF (SF_URBAN_PHYSICS == 2) THEN
       ...
       CALL BEP(frc_urb2d,utype_urb2d,itimestep,dz8w,dt,u_phy,v_phy,   &
                ...
                rl_up_urb,RL_UP_URB0,rs_abs_urb,                      &
				emiss_urb,EMISS_URB0,qsfc,grdflx_urb,                  &
                ids,ide, jds,jde, kds,kde,                             &
                ims,ime, jms,jme, kms,kme,                             &
                its,ite, jts,jte, kts,kte )
    ...
    if((sf_urban_physics.eq.2).OR.(sf_urban_physics.eq.3))then         
         ...
         do k=kts,kte
            ...
            tloc_save=int(time_h)
            
            
            xut=omg_urb2d(i,j)/3.14159265358979324*180./15.+12.+0.5
            tloc=int(xut-(floor(xut/24)*24.))

            
            
            
            if(tloc==0) tloc=24
            if((tloc <= 0) .or. (tloc > 24)) then
                  write(90,*) i,j,OMG_URB2D(I,J),tloc_save,time_h,tloc
                  tloc = 12
            endif
            ...
            anthro=0.
            if(UTYPE_URB2D(I,J)==1) anthro=40.
            if(UTYPE_URB2D(I,J)==2) anthro=20.
            if(UTYPE_URB2D(I,J)==3) anthro=10.

           
           
            
            placeholder=(1.-frc_urb2d(i,j))*hfx_rural(i,j)/dz8w(i,1,j)/rho(i,1,j)/CP + &
            b_t_bep(i,1,j)+frc_urb2d(i,j)*anthro*anth_diurnal(tloc)/dz8w(i,1,j)/rho(i,1,j)/CP  
            b_t_bep(i,1,j) = placeholder
            placeholder=(1.-frc_urb2d(i,j))*qfx_rural(i,j)/dz8w(i,1,j)/rho(i,1,j)+b_q_bep(i,1,j)
            b_q_bep(i,1,j)=placeholder
           ...
           IF (FRC_URB2D(I,J).GT.0.) THEN
			  ...
			  RL_UP_TOT0=(1.-frc_urb2d(i,j))*rl_up_rural+frc_urb2d(i,j)*rl_up_urb0(i,j)   
              emiss(i,j)=(1.-frc_urb2d(i,j))*emiss_rural(i,j)+frc_urb2d(i,j)*emiss_urb(i,j)
			  EMISS0(I,J)=(1.-frc_urb2d(i,j))*emiss_rural(i,j)+frc_urb2d(i,j)*emiss_urb0(i,j) 
              ...
              tsk0(i,j)=(max(0., (-1.*rl_up_tot0-(1.-emiss0(i,j))*glw(i,j) )/emiss0(i,j)/sigma_sb))**.25 
