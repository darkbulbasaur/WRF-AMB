 ...
 USE module_sf_urban
   ...
   CONTAINS
      ...
      subroutine BEP(FRC_URB2D,UTYPE_URB2D,itimestep,dz8w,dt,u_phy,v_phy,      &
                      ...
                      rl_up,RL_UP0,rs_abs,emiss,EMISS0,qsfc,grdflx_urb, &
   ...
   REAL, DIMENSION( ims:ime,jms:jme), INTENT(IN) :: hgt_urb2d
	  ...
	  REAL RL_UP0(ITS:ITE,JTS:JTE)
	  REAL EMISS0(ITS:ITE,JTS:JTE)
      REAL QSFC(ITS:ITE,JTS:JTE)
	 ...
	 enddo
         ...
         call BEP1D(iurb,kms,kme,kts,kte,z1D,dt,ua1D,va1D,pt1D,da1D,pr1D,pt01D,  &
				   ...
				   rl_up0(ix,iy),emiss0(ix,iy), QSFC(ix,iy),         & 
	 ...
	 enddo
      ...
      call modif_rad(iurb,ndu,nzu,z_u,ws,               &
       ...
       call upward_rad(ndu,nzu,ws,bs,                   &
                       ...
                       rs_abs,rl_up,RL_UP0,emiss,EMISS0,grdflx_urb)               


     

      tt = pt_u(1)*(pr_u(1)/1E5)**(2/7)
      qq = q * 1000
      prop = -3.796 + 3.394E-2*qq + 2.543E-2*tt - 3.039E-4*qq**2 - 5.943E-5*qq*tt - 4.1E-5*tt**2
      call surf_temp(prop,nzu,ndu,pr_u,dt,ss,              & 
      ...
      subroutine surf_temp(prop,nz_u,nd,pr,dt,ss,rs,rl,rsg,rlg,rsw,rlw, &
                          tg,alag,csg,emg,albg,ptg,sfg,gfg,             &
                          tr,alar,csr,emr,albr,ptr,sfr,gfr,             &
                          tw,alaw,csw,emw,albw,ptw,sfw,gfw)             
      ...
      do id=1,nd
       ...
       do iz=2,nz_u
		...
		if((rl-5.67E-8*tr_tmp(nwr_u)**4).eq.0.) then
		emr = 10.
		else
		emr = (rl*prop-3.1416*(3.313E-3*tr_tmp(nwr_u)**2 - 1.209*tr_tmp(nwr_u)+117.4))/(rl-5.67E-8*tr_tmp(nwr_u)**4)
		end if
...
'singular matrix in gaussj')
         ...
         if(a(icol,icol).eq.0) CALL wrf_error_fatal3("<stdin>",2309,&
'singular matrix in gaussj')
 ...
 10      continue
      ...
      do id=1,ndu          
	   ...
	   rl_emit0=rl_emit0-( emg_u*sigma*(tg(id,ng_u)**4.)+(1-emg_u)*rlg(id))*ws(id)/(ws(id)+bs(id))/ndu
       ...
       rs_abs=rs_abs+(1.-albg_u)*rsg(id)*ws(id)/(ws(id)+bs(id))/ndu
         ...
         grdflx_urb=grdflx_urb-gfl*ws(id)/(ws(id)+bs(id))/ndu  
          ...
          do iz=2,nzu
			...
			RL_EMIT0=rl_emit0-(emr_u0*sigma*(tr(id,iz,nwr_u)**4.)+(1-emr_u0)*rld)*ss(iz)*bs(id)/(ws(id)+bs(id))/ndu
         ...
         do iz=1,nzu            
			...
			rl_emit0=rl_emit0-(emw_u*sigma*( tw(2*id-1,iz,nwr_u)**4.+tw(2*id,iz,nwr_u)**4. )+          &
               (1-emw_u)*( rlw(2*id-1,iz)+rlw(2*id,iz) ) )*dz_u*pb(iz+1)/(ws(id)+bs(id))/ndu
      ...
      enddo
        ...
        emiss=(emg_u+emw_u+1)/3.
		EMISS0=(EMG_U+EMW_U)/3.
        rl_up=(rl_inc+rl_emit)-rld
        RL_UP0=(RL_INC+RL_EMIT0)-RLD
