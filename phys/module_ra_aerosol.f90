    ...
    aer_aod550_opt_select: select case(aer_aod550_opt)
       ...
       case(1)
          if (aer_aod550_val .lt. 0 ) then
             ...
             call wrf_error_fatal3("<stdin>",130,&
wrf_err_message)
          end if
...
'Aerosol single-scattering albedo must be within [0,1]. ' // &
          ...
          write( wrf_err_message, '("aer_ssa_opt=",I1, &
                                  ": single-scattering albedo read from auxinput (min=",F6.3," max=",F6.3,")")') &
...
wrf_err_message)
          ...
          write( wrf_err_message, '("aer_ssa_opt=",I1, &
                                    ": single-scattering albedo fixed to value ",F6.3)') aer_ssa_opt,aer_ssa_val
          call wrf_debug(100, wrf_err_message )
          do j=jts,jte
             do i=its,ite
                do k=kts,kte
