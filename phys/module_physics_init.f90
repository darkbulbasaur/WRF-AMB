...
CONTAINS
   ...
   SUBROUTINE phy_init ( id, config_flags, DT, restart, zfull, zhalf,     &
                         ...
                         TS_URB2D, FRC_URB2D,URBVEG2D,UTYPE_URB2D, & 
        ...
        DO ls=1,luseas
          ...
          DO LC=1,LUCATS
            IF(found_lu)THEN
              ...
              IF(LC.NE.LI)CALL wrf_error_fatal3("<stdin>",1390,&
...
'Need CAM or RRTMG radiation for bucket_J option')
  ...
  if ( config_flags%fractional_seaice == 0 ) then
   ...
   sfc_select: SELECT CASE(config_flags%sf_surface_physics)
      ...
      CALL wrf_debug(200,'TJ before urban var init')
      ...
      CALL wrf_debug(200,'TJ after urban var init')
             ELSE
