...
!ENDOFREGISTRYGENERATEDINCLUDE
   ...
   IF ( ( MOD (ide-ids,config_flags%parent_grid_ratio) .NE. 0 ) .OR. &
      ...
      CALL wrf_error_fatal3("start_em.b",143,&
message )
   END IF
   ...
   IF ( config_flags%polar ) THEN
     ...
     IF ( alloc_status .NE. 0 ) THEN
        ...
        CALL wrf_error_fatal3("start_em.b",161,&
'Error allocating entire domain size of 2d array CLAT for global domain' )
   ...
   IF ( ( grid%id == 1 ) .AND. &
     ...
     IF ( (grid%stoch_force_opt==1) .or. (grid%perturb_bdy==1)) then
     grid%SPTFORCC=0.0
     grid%SPTFORCS=0.0
...
TRIM(wrf_err_message))
   ...
   ELSE
      ...
      IF ( ( t00 .LT. 100. .or. p00 .LT. 10000.) .AND. ( .NOT. grid%this_is_an_ideal_run ) ) THEN
         ...
         CALL wrf_error_fatal3("start_em.b",465,&
TRIM(wrf_err_message))
      ENDIF
   ...
   IF ( ( tiso_tmp .NE. tiso ) .AND. ( .NOT. grid%this_is_an_ideal_run ) ) THEN
      ...
      CALL wrf_error_fatal3("start_em.b",477,&
TRIM(wrf_err_message))
   ENDIF
   ...
   IF ( .NOT. config_flags%restart .AND. &
      ...
      IF ( config_flags%map_proj .EQ. 0 ) THEN
         CALL wrf_error_fatal3("start_em.b",485,&
   ...
   DO ij = 1, grid%num_tiles
    ...
    IF ( ( grid%dfi_opt .EQ. DFI_NODFI ) .or. &
     ...
     CALL phy_init (  grid%id , config_flags, grid%DT, grid%RESTART, grid%znw, grid%znu,   &
                      ...
                      grid%TS_URB2D, grid%FRC_URB2D, grid%URBVEG2D, grid%UTYPE_URB2D,  & 
