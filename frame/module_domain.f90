...
ENDIF
IF ( ASSOCIATED( grid%tsk0 ) ) THEN 
  DEALLOCATE(grid%tsk0,STAT=ierr)
 if (ierr.ne.0) then
 CALL wrf_error_fatal3("<stdin>",4428,&
'frame/module_domain.f: Failed to deallocate grid%tsk0. ')
 endif
ENDIF
IF ( ASSOCIATED( grid%emiss0 ) ) THEN 
  DEALLOCATE(grid%emiss0,STAT=ierr)
 if (ierr.ne.0) then
 CALL wrf_error_fatal3("<stdin>",4435,&
'frame/module_domain.f: Failed to deallocate grid%emiss0. ')
 endif
ENDIF
...
ENDIF
IF ( ASSOCIATED( grid%urbveg2d ) ) THEN 
  DEALLOCATE(grid%urbveg2d,STAT=ierr)
 if (ierr.ne.0) then
 CALL wrf_error_fatal3("<stdin>",5303,&
'frame/module_domain.f: Failed to deallocate grid%urbveg2d. ')
 endif
ENDIF
   ...
   END SUBROUTINE init_module_domain
      ...
      SUBROUTINE domain_clock_get( grid, current_time,                &
        ...
        IF ( PRESENT( time_stepstr ) ) THEN
          ...
          IF ( rc /= WRFU_SUCCESS ) THEN
            CALL wrf_error_fatal3("<stdin>",15007,&
   ...
   SUBROUTINE get_dims_from_grid_id (  id   &
    ...
    INTEGER, DIMENSION(3), INTENT(INOUT) ::                   &
     ...
     ELSE
        ...
        CALL wrf_error_fatal3("<stdin>",15830,&
TRIM(mess))
     ENDIF
